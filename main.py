...
target_classes = ['car', 'bus', 'truck', 'person','gun']
            ...
            if center_x is not None and center_y is not None:
                ...
                if inside_polygon((center_x, center_y), np.array([pts])) and name == 'person':
                    ...
                    if count < number_of_photos:
                        cv2.imwrite("detected" + str(count) +
                                    ".jpg", frame_detected)
                    # Playing the alarm
...
if msg_flag:
    token = '6475709038:AAG5mge8elt7JKtfido4v5rt9odRh2hExuE'
    receiver_id = 2078149930
    bot = telepot.Bot(token)
    bot.sendMessage(receiver_id, 'terrorist detected')

    geolocator = Nominatim(user_agent="MyApp")
    location = geolocator.geocode("trichy")

    loc=str("latitude: "+str(location.latitude)+" longitude: "+str(location.longitude))
    bot.sendMessage(receiver_id,loc)
    
    bot.sendPhoto(receiver_id, photo=open('detected0.jpg', 'rb'))
    bot.sendPhoto(receiver_id, photo=open('detected1.jpg', 'rb'))
    bot.sendPhoto(receiver_id, photo=open('detected2.jpg', 'rb'))

    top = Tk()
    top.geometry("100x100")
    messagebox.showinfo("Location coordinates", loc)
    top.mainloop()
